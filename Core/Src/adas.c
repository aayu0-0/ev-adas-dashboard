...
void ADAS_Update(ADAS_HandleTypeDef *adas, EV_HandleTypeDef *ev)
{
    ...
    /* Step 3: Forward Collision Warning — only meaningful once the vehicle
     * has actually entered DRIVING/REGEN. Without this gate, a stationary
     * car in PARKED/READY (speed frozen, possibly even 0) could still
     * trip a critical collision warning purely from a nearby object at
     * boot/idle, before it had moved at all. TTC itself is already safe
     * at rest (forced to 99.9s when speed_ms <= 0.5, see Step 2 above);
     * this additionally suppresses the raw front_cm proximity check from
     * firing on its own while not actually driving. */
    uint8_t vehicle_driving = (ev->state == STATE_DRIVING) ||
                               (ev->state == STATE_REGEN);

    uint8_t fcw_crit = 0, fcw_warn = 0;
    if (vehicle_driving) {
        fcw_crit = (adas->front_cm < ADAS_FCW_CRIT_CM)
                || (adas->ttc_sec  < ADAS_TTC_CRIT_S);
        fcw_warn = (adas->front_cm < ADAS_FCW_WARN_CM)
    ...
    /* Step 6: Parking Assist — active below the parking speed gate.
     * All three sensors feed in; score is driven by whichever is closest
     * (the most relevant obstacle), 0–100 where 100 = touching, 0 = clear
     * (>= ADAS_PARK_MAX_CM away). No hysteresis here — this is an advisory
     * proximity readout, not an alarm that needs de-bounce. */
    adas->parking_active = (ev->speed_kmh < ADAS_PARK_SPEED_KMH);

    if (adas->parking_active) {
        float closest_cm = adas->front_cm;
        if (adas->left_cm  < closest_cm) closest_cm = adas->left_cm;
        if (adas->right_cm < closest_cm) closest_cm = adas->right_cm;

        float score = (1.0f - (closest_cm / ADAS_PARK_MAX_CM)) * 100.0f;
