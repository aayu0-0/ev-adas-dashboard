...
void EV_Init(EV_HandleTypeDef *ev)
{
    ...
    ev->speed_kmh = 0.0f;   /* was hardcoded to 10 — caused false collision
                                warnings at boot before the car had even
                                moved, since ADAS_Update()/Fault_Check()
                                run unconditionally every 100ms regardless
                                of vehicle state */
    ev->state      = STATE_PARKED;   /* explicit — memset(0) happens to match,
                                         but spell it out so intent is clear */
}
...
{
    /* ── 0. Vehicle FSM — gates whether physics runs this tick ────────────── *
     * PARKED : torque/regen forced to 0, no physics, waits for accel > 2%
     * READY  : waits for accel > 5% before advancing to DRIVING
     * DRIVING: normal physics; brake > 10% → REGEN
     * REGEN  : regen physics (handled by the brake check below); brake
     *          releasing below 5% → back to DRIVING
     * FAULT  : torque forced to 0, speed frozen, no physics — only
     *          Fault_Clear() (fault.c) can exit this state
     * ──────────────────────────────────────────────────────────────────── */
    switch (ev->state)
    {
        case STATE_PARKED:
            ...
            ev->regen_level  = 0.0f;
            if (ev->accel_pedal > EV_READY_PEDAL_PCT)
                ev->state = STATE_READY;
            return;  /* skip physics while parked */

        case STATE_READY:
            if (ev->accel_pedal > EV_DRIVING_PEDAL_PCT)
                ev->state = STATE_DRIVING;
            return;  /* skip physics until accel crosses the driving threshold */

        case STATE_DRIVING:
            if (ev->brake_pedal > EV_REGEN_ENTER_PCT)
                ev->state = STATE_REGEN;
            break;  /* fall through to physics below */

        case STATE_REGEN:
            if (ev->brake_pedal < EV_REGEN_EXIT_PCT)
                ev->state = STATE_DRIVING;
            break;  /* fall through — regen torque computed in step 2 */

        case STATE_FAULT:
            ev->motor_torque = 0.0f;
            /* speed_kmh intentionally left untouched — freezes in place
               since no integration happens below this point this tick */
            return;  /* freeze everything; only Fault_Clear() can exit FAULT */

        default:
            ev->state = STATE_PARKED;  /* unreachable in practice — defensive */
            return;
    }
    ...
    /* ── 1. Motor torque from accelerator ─────────────────────────────────── */
         ...
         * Torque only goes negative while the FSM says we're actually in
         * REGEN — using ev->state here (not a separate threshold) keeps the
         * physics and the state machine in agreement: no band where the
         * vehicle is "in DRIVING" per the FSM but already producing regen
         * torque per the math. */
        if (ev->state == STATE_REGEN) {
              /* 0–70 % scale */
